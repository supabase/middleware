...
The `yield` expression resolves to the downstream `Response` (typed as `Response`, inferred — no annotation). Yield exactly once: yield the contribution to fall through, or yield a `Response` to short-circuit. `try/finally` around the `yield` gives request-spanning cleanup; `try/catch` can turn a downstream throw into a `Response`.

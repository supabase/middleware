/**
 * Integration tests for the postgres middleware. These need a REAL Postgres —
 * RLS, roles, and `set local role` can't be faked by an in-memory shim — so the
 ...
 * - table `pg_mw_notes(id, owner uuid default auth.uid(), body)` with RLS on
...
/** Stand-in base context — seeded automatically at the entry call in production. */
...
const SETUP_SQL = `
  ...
  drop table if exists pg_mw_notes;
  create table pg_mw_notes (
    id uuid primary key default gen_random_uuid(),
    owner uuid not null default auth.uid(),
    body text
  );
  alter table pg_mw_notes enable row level security;
  grant select, insert, update, delete on pg_mw_notes to authenticated, anon;
  create policy pg_mw_notes_owner on pg_mw_notes
...
function authedCtx(sub: string): { jwtClaims: JWTClaims } & BaseContext {
  return { jwtClaims: { sub, role: 'authenticated' }, _runtime: runtime }
}

const req = (): Request => new Request('http://localhost/')

describe.skipIf(!DB_URL)('postgres middleware (integration)', () => {
  let pool: Pool
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `insert into pg_mw_notes (owner, body) values ($1, 'a1'), ($1, 'a2'), ($2, 'b1')`,
      [USER_A, USER_B],
    )
  })
  ...
  it('scopes ctx.postgres.db to the caller — other users rows are invisible', async () => {
    const handler = withPostgres({ pool }, async (_req, ctx) => {
      const r = await ctx.postgres.db.query(
        'select body from pg_mw_notes order by body',
      )
  ...
  it('ctx.postgres.db.tx commits a multi-statement block atomically', async () => {
    const handler = withPostgres({ pool }, async (_req, ctx) => {
      await ctx.postgres.db.tx(async (c) => {
        await c.query(`insert into pg_mw_notes (body) values ('tx1')`)
        await c.query(`insert into pg_mw_notes (body) values ('tx2')`)
      })
      const r = await ctx.postgres.db.query(
        `select count(*)::int as n from pg_mw_notes where body in ('tx1', 'tx2')`,
      )
  ...
  it('ctx.postgres.db.tx rolls the whole block back when fn throws', async () => {
    const handler = withPostgres({ pool }, async (_req, ctx) => {
      try {
        await ctx.postgres.db.tx(async (c) => {
          await c.query(`insert into pg_mw_notes (body) values ('rollback-me')`)
      ...
      const r = await ctx.postgres.db.query(
        `select count(*)::int as n from pg_mw_notes where body = 'rollback-me'`,
      )
      return Response.json({ n: r.rows[0].n })
    })
  ...
  it('ctx.postgres.adminDb bypasses RLS and sees every row when admin: true', async () => {
    const handler = withPostgres({ pool, admin: true }, async (_req, ctx) => {
      const r = await ctx.postgres.adminDb.query(
        `select count(*)::int as n from pg_mw_notes where body in ('a1', 'a2', 'b1')`,
      )
      return Response.json({ n: r.rows[0].n })
    })

    const res = await handler(req(), { jwtClaims: null, _runtime: runtime })
  ...
  it('null jwtClaims runs as anon — owner-scoped policy yields no rows', async () => {
    const handler = withPostgres({ pool }, async (_req, ctx) => {
      const r = await ctx.postgres.db.query(
        'select count(*)::int as n from pg_mw_notes',
      )
      return Response.json({ n: r.rows[0].n })
    })

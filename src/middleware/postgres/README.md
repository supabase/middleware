...
This package is **auth-agnostic** — it doesn't ship an auth middleware (JWT
verification belongs in your auth layer, e.g. [`@supabase/server`](https://github.com/supabase/server)).
Provide `jwtClaims` from any upstream middleware that contributes it. The smallest
possible upstream decodes the token and contributes the claims directly:

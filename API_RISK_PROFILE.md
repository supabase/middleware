...
| **R8** | ✅ **resolved** (by scoping) | Auth is **not** core's job. An `/auth` (`withAuth`) middleware was shipped then **removed** (along with `/auth-hook`) — JWT verification is Supabase-specific and owned by `withSupabase` (`supabase/server`, which uses `jose`). web-middleware stays auth-agnostic: `withPostgres` declares `In: { jwtClaims }`; the consumer's auth middleware contributes it. `withPostgres` reads `SUPABASE_DB_URL` via `ctx._runtime.getEnv`. See [docs/solutions/R8-auth-middleware.md](docs/solutions/R8-auth-middleware.md) (superseded).                                            | n/a — middleware removed; `withPostgres` prerequisite enforcement still tested.                                                                       |

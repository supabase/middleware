...
| **R3** | ⚪ **reverted by design**   | Removing `toFetch` removed its `onError` boundary. Errors again propagate to the host (which returns its default 500). No silent swallowing. A dedicated error-boundary middleware (`withCatch`) is the future path; `auth-hook`'s `JSON.parse` throw is the concrete case to cover.                                                                                              | n/a                                                                                                                                                   |
...
- **R2 — one-shot body.** Untouched and now the **top remaining risk**. Still no
  buffered-body/clone convention; two body-reading middleware still can't compose.
  Recommended next: an opt-in `ctx.rawBody` populated once, or a documented
  `req.clone()` authoring rule.
- **R3 — error containment.** Reverted with `toFetch`'s removal; add a `withCatch`
  middleware if a defined failure response is wanted.

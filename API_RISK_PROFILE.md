...
3. **A `Runtime` abstraction at `ctx._runtime`** — `{ name: RuntimeName; getEnv(key) }`
...
| ------ | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **R2** | ✅ **resolved**                           | The body is made re-readable on **`req`** itself: the entry wraps the request with a `bufferRequest` proxy whose body methods read-once-cache. `auth-hook` reads `req.text()`, and a downstream handler can read the body again — no `ctx` key needed (avoids the top-level asymmetry). See [docs/solutions/R2-buffered-body.md](docs/solutions/R2-buffered-body.md).                                                                                                                                                                                                         | `define-middleware.test.ts` "req body is readable from multiple layers"; `pnpm test` ✅.                                                              |
...
| **R4** | ✅ **resolved**                           | Accidental same-key reuse is a **compile error** (no more silent shadowing); _intentional_ multi-instance is supported via `.as(newKey)`, which every `defineMiddleware` middleware exposes to re-key its contribution (`withFeatureFlag.as('beta')(…)`). See [docs/solutions/R4-multi-instance.md](docs/solutions/R4-multi-instance.md).                                                                                                                                                                                                                                     | `define-middleware.test.ts` (runtime two-instance + tsc-verified distinct typed keys); collision still errors under `satisfies`.                      |
| **R1** | ✅ **resolved**                           | **Arg 2:** the wrapper distinguishes an upstream context from a host platform arg (`isContext`) and seeds `{ _runtime }` rather than merging it — so a 2nd-arg `env` never reaches `ctx`, regardless of enumerability. `getEnv` is now parametrized by runtime and **unit-tested per branch** (incl. the Cloudflare per-request-env path that was previously unverified). **Arg 3:** a host-supplied third argument (the Workers `ExecutionContext`) is not honored — ignored with a one-time `console.warn` rather than dropped silently (the Deno target never passes one). | `runtime.test.ts` (`makeGetEnv` per runtime + `detectRuntimeName`); `define-middleware.test.ts` (arg-2 no-leak; arg-3 throws).                        |
| **R3** | ✅ **resolved** (out of scope)            | Error containment isn't the substrate's job. A `withCatch` wrapper was tried then **removed** — it was a near-empty `.catch()` decorator with no composition machinery. A complete handler/middleware owns its own errors: `try/catch` in the handler, or `stack(req).catch(onError)` at the entry. Documented in the README "Request-side only" section. See [docs/solutions/R3-error-boundary.md](docs/solutions/R3-error-boundary.md) (superseded).                                                                                                                        | n/a — wrapper removed; pattern documented.                                                                                                            |
| **R8** | ✅ **resolved** (by scoping)              | The whole RLS-Postgres-needs-auth concern left the substrate: `/auth`, `/auth-hook`, **and** `withPostgres` are all **removed** from web-middleware. Auth and raw-pg RLS are Supabase-specific and live in `@supabase/server` (auth via `jose`; `withPostgres` → `@supabase/server/postgres`, built on `defineMiddleware`, `pg` optional peer). web-middleware is now the auth-agnostic, edge-portable substrate (core + `feature-flag`). See [docs/solutions/R8-auth-middleware.md](docs/solutions/R8-auth-middleware.md) (superseded).                                      | n/a — middleware extracted; the `In: { jwtClaims }` prerequisite pattern is still exercised by the core type-guarantee tests.                         |
...
| **R9** | ✅ **resolved** (no shared helper needed) | A shared `RejectConfig` + `rejection()` helper was tried, but once `auth-hook` was dropped, `feature-flag` was the only short-circuiting middleware left — one consumer doesn't justify a shared helper. Reverted: `feature-flag` just returns `Response.json(...)` inline (middleware short-circuit by returning a `Response`). `ctx` name kept by design. See [docs/solutions/R9-reject-convention.md](docs/solutions/R9-reject-convention.md) (superseded).                                                                                                                | `pnpm test` ✅; feature-flag reject tests (default + custom status/body) still pass inline.                                                           |
...
- **`ctx` naming.** Kept `ctx` (not `state`/`data`) — renaming is a breaking,
  ...
  `ctx._runtime` / `ctx.body` documented. See R9 solution doc.
